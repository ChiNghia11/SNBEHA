...
// Thêm đoạn này vào cuối file script.js để xử lý khi xoay điện thoại
window.addEventListener("resize", () => {
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
});

// Chỉnh lại thông số hạt bay trong hàm startFX để mượt hơn trên mobile ngang
function startFX() {
  // Giảm tốc độ gió (wind) một chút để hạt không bay quá xéo khi màn hình ngang dài
  for (let i = 0; i < 60; i++) {
    // 60 hạt là vừa đủ cho điện thoại
    particles.push({
      x: Math.random() * canvas.width,
      y: Math.random() * -canvas.height,
      size: Math.random() * 10 + 12,
      speed: Math.random() * 3 + 2, // Tốc độ vừa phải
      wind: Math.random() * 2 - 1, // Gió nhẹ nhàng
      emoji: petals[Math.floor(Math.random() * petals.length)],
      rotation: Math.random() * 360,
